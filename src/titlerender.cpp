...
void titlerender(Graphics& dwgfx, mapclass& map, Game& game, entityclass& obj, UtilityClass& help, musicclass& music)
{
    ...
    else
    {
		...
		else if (game.currentmenuname == "controller")
		{
			...
			{
				dwgfx.Print( -1, 85, "Flip is bound to: FIX LATER?" /*+ std::string(UtilityClass::GCString(game.controllerButton_flip)) */, tr, tg, tb, true);
				dwgfx.Print( -1, 95, "Enter is bound to: FIX LATER?"/*  + std::string(UtilityClass::GCString(game.controllerButton_map))*/, tr, tg, tb, true);
				dwgfx.Print( -1, 105, "Menu is bound to: FIX LATER?"/* + std::string(UtilityClass::GCString(game.controllerButton_esc)) */, tr, tg, tb, true);
			}
